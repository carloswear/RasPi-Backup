...
# --- 脚本配置变量 ---
BACKUP_DIR="/media/8T/4Backups/Backup"     # 备份镜像文件保存目录
...
# --- 权限检查 ---
if [ "$(id -u)" -ne 0 ]; then
    echo "此脚本必须以 root 用户身份运行！请使用 sudo $0"
    exit 1
fi
...
if [ -z "$ROOT_DEVICE_PARTITION" ]; then
    echo "错误：无法确定根目录 / 的挂载分区。"
    exit 1
fi

# 解析设备名，支持 mmcblk、nvme、sdX
if [[ "$ROOT_DEVICE_PARTITION" =~ ^(/dev/mmcblk[0-9]+)p[0-9]+$ ]]; then
    ...
    echo "错误：无法解析根目录所在设备类型。"
    exit 1
fi

echo "检测到系统盘设备：$SD_DEVICE"
...
BOOT_MOUNTPOINT=$(echo "$PART_INFO" | jq -r ".blockdevices[] | (.children[]? // .) | select(.name == \"$(basename "$BOOT_PART")\") | .mountpoints[0] // \"\"")
ORIG_BOOT_PARTUUID=$(echo "$PART_INFO" | jq -r ".blockdevices[] | (.children[]? // .) | select(.name == \"$(basename "$BOOT_PART")\") | .partuuid // \"\"")
ORIG_ROOT_PARTUUID=$(echo "$PART_INFO" | jq -r ".blockdevices[] | (.children[]? // .) | select(.name == \"$(basename "$ROOT_PART")\") | .partuuid // \"\"")
BOOT_FSTYPE=$(echo "$PART_INFO" | jq -r ".blockdevices[] | (.children[]? // .) | select(.name == \"$(basename "$BOOT_PART")\") | .fstype // \"\"")
ROOT_FSTYPE=$(echo "$PART_INFO" | jq -r ".blockdevices[] | (.children[]? // .) | select(.name == \"$(basename "$ROOT_PART")\") | .fstype // \"\"")
...
echo "引导分区文件系统类型: $BOOT_FSTYPE"
echo "根分区文件系统类型: $ROOT_FSTYPE"

# --- 选择备份模式 ---
echo "--- 阶段 3.5/8: 选择备份模式 ---"
echo "备份模式说明："
echo "F - Full 全盘镜像备份（适用于完整克隆）"
echo "S - Slim 精简备份（仅复制已用空间 + 20%，节省存储空间）"
read -p "请选择备份模式 [F/S] (默认S): " backup_mode
backup_mode=${backup_mode^^}
[ "$backup_mode" != "F" ] && backup_mode="S"

if [[ "$backup_mode" == "F" ]]; then
    echo "选择：全盘备份模式"
    IMAGE_SIZE_KB=$(lsblk -b -n -o SIZE "$SD_DEVICE" | awk '{print int($1 / 1024)}')
else
    echo "选择：精简备份模式（已用空间 + 20%）"
...
echo "预计镜像大小约：$((IMAGE_SIZE_KB / 1024)) MB"

read -p "请确认以上信息无误，即将开始备份。确认请按 y，否则退出 (y/N): " confirm
if [[ ! "$confirm" =~ ^[Yy]$ ]]; then
    echo "用户取消备份。"
    exit 0
fi
...
# --- 创建分区表 ---
echo "--- 阶段 6/8: 创建分区表 ---"
# 此处简单分配：boot分区4MiB起，大小256MiB，剩余为root分区
parted "$DEST_IMG_PATH" --script -- mklabel msdos
parted "$DEST_IMG_PATH" --script -- mkpart primary fat32 4MiB 256MiB
parted "$DEST_IMG_PATH" --script -- mkpart primary ext4 256MiB 100%
echo "分区表创建完成。"

# --- 挂载镜像文件并格式化分区 ---
echo "--- 阶段 7/8: 挂载镜像并格式化分区 ---"
...
# --- 挂载镜像分区 ---
echo "--- 阶段 8/8: 挂载镜像分区并复制数据 ---"
mkdir -p "$MOUNT_BOOT" "$MOUNT_ROOT"

mount "$partBoot" "$MOUNT_BOOT"
if ! mountpoint -q "$MOUNT_BOOT"; then
    echo "错误：引导分区挂载失败。"
    exit 1
fi

mount "$partRoot" "$MOUNT_ROOT"
...
echo "✅ 备份完成，镜像文件路径：$DEST_IMG_PATH"
# 脚本结束，trap 自动调用 cleanup
